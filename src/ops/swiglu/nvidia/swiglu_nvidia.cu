...
template <typename T>
__global__ void swiglu_kernel(T *out, const T *gate, const T *up, size_t seq_len, size_t inter_size) {
    size_t idx = static_cast<size_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    size_t total = seq_len * inter_size;
    if (idx >= total) {
        return;
    }
    float gate_v = detail::to_float(gate[idx]);
    float up_v = detail::to_float(up[idx]);
    float out_v = up_v * gate_v / (1.0f + expf(-gate_v));
    out[idx] = detail::from_float<T>(out_v);
}
